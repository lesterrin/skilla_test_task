...
import { ORDER_TYPES, SORT_BY_TYPE } from "shared/api/calls/types"
...
import styles from "pages/CallsTablePage/components/Table/TableHead/TableHead.module.css"
  ...
  return (
    <thead>
      <tr className={styles.tableHeadRow}>
        ...
        <td
          className={classCombiner(styles.dateColumn, styles.pointer)}
          onClick={() => onSortedColumnClick(SORT_BY_TYPE.DATE)}
        >
          Время {isDateSortActive && orderArrow}
        </td>
        ...
        <td
          className={classCombiner(styles.durationColumn, styles.pointer)}
          onClick={() => onSortedColumnClick(SORT_BY_TYPE.DURATION)}
        >
          Длительность {isDurationSortActive && orderArrow}
        </td>
      </tr>
    </thead>
  )
